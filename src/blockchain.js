...
class Blockchain {
    ...
    requestMessageOwnershipVerification(address) {
        ...
        return new Promise((resolve) => {
            ...
            let uMess = `${address}:${new Date().getTime().toString().slice(0,-3)}:starRegistry`;
            resolve(uMess);
        });
    }
    ...
    submitStar(address, message, signature, star) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            if((currentTime - messageTime)/1000/60 < 5){
            ...
            self.chain.forEach( async(block) =>{
                ...
                if(await block.validate()){
                    if(block.height > 0 ){ //not genesis block
                        let prevHash = self.chain.filter(b => b.height === block.height - 1)[0];
                        if (block.previousBlockHash != prevHash){
                            errorLog.push("Block #" + block.height + "previous hash don't match");
                        }
                    } else {
